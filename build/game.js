...
document.addEventListener('keydown', (event) => {
    ...
    switch (event.key) {
        case "ArrowUp":
            princess.style.top = `${parseInt(princess.style.top) - modifier - cloud}px`;
            break;
        case "ArrowDown":
            princess.style.top = `${parseInt(princess.style.top) + modifier + cloud}px`;
            break;
        case "ArrowLeft":
            princess.style.left = `${parseInt(princess.style.left) - modifier - cloud}px`;
            break;
        case "ArrowRight":
            princess.style.left = `${parseInt(princess.style.left) + modifier + cloud}px`;
            break;
...
function movePicture(e) {
    ...
    switch (e.key) {
        case "ArrowUp":
            if (currentCollectorIndex % height < height - 1)
                currentCollectorIndex -= 1;
            break;
        case "ArrowDown":
            if (currentCollectorIndex % height < height - 1)
                currentCollectorIndex += 1;
            break;
        case "ArrowLeft":
            if (currentCollectorIndex % width !== 0)
                currentCollectorIndex -= 1;
            break;
        case "ArrowRight":
            if (currentCollectorIndex % width < width - 1)
                currentCollectorIndex += 1;
            break;
    }
    squares[currentCollectorIndex].classList.remove('PrincessPeach');
